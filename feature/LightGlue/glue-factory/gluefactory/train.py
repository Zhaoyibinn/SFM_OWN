...
def training(rank, conf, output_dir, args):
    ...
    if init_cp is not None:
        local_path = '/home/zhaoyibin/3DRE/sfm-learn/SFM_OWN/feature/LightGlue/wight/superpoint_lightglue_v0-1_arxiv.pth'  # 赵逸彬修改，读取官方weight
    ...
    for i in range(9):
        ...
        pattern = f"cross_attn.{i}", f"matcher.transformers.{i}.cross_attn"
        state_dict = {k.replace(*pattern): v for k, v in state_dict.items()}
    model.load_state_dict(state_dict, strict=False)
